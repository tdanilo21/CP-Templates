...
private:
    ...
    void init(int n){
        ...
        this->par = vector<vector<int> >(n, vector<int>((int)log2(n)+10, -1));
...
public:
    ...
    void Compute(int root){
        ...
        dfs(root, -1, 0);
        for (int d = 1; d < (int)log2(this->n)+1; d++)
    ...
    int GetPar(int s, int d) const {
        ...
        if (bitcnt(d) == 1) return this->par[s][(int)log2(d)];
        return GetPar(this->par[s][(int)log2(d)], d-(int)log2(d));
    }
